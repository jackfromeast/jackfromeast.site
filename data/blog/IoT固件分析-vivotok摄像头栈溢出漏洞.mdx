...
<mark>**补充:**</mark> 魔术数字也会在文件中使用，用于识别一个文件格式或协议类型的一段常量或字符串，例如UNIX的特征签名。在特定文件格式中加入固定数值和固定字符串，然后便可以通过检查文件是否包含这些数据来快速地识别文件格式。例如：GIF文件开头会包含GIF89a（47 49 46 38 39 61）或GIF87a（47 49 46 38 37 61）这两种字符串。
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/binwalk_install.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/dir-L1.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/squashfs-root.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/qemu_install.png")
...
qemu-
```
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/qemu-.png")
注意其中同样是arm框架，包含了三种命令：qemu-arm, qemu-system-arm, qemu-arm-static.下面对这三种模式进行解读：
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/cp_qemu_arm_static.png")
...
```
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/3-2.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/3-3.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/3-4.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/3-5.png")

重新使用qemu-arm-static运行，发现boa简易webserver已经成功提供http服务。

![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/3-6.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-1.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-2.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-3.png")
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-4.png")
在qemu上使用ftp远程登陆ubuntu。
```
ftp 192.168.2.1
```

![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-5.png")

在ubuntu上打包squashfs-root目录为tar,然后在qemu虚拟机上使用get命令下载。

![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-6.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-7.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/4-8.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/5-3.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/5-4.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/5-5.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/6-1.png")

宿主机上连接该端口。

![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/6-2.png")
...
![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/6-3.png")
在宿主机上输入continue，继续程序运行如下所示，发现调试程序崩溃，发生了段错误！

![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/6-4.png")
在qemu的虚拟机上发现httpd服务报错退出。

![]("/static/images/IoT固件分析-vivotok摄像头栈溢出漏洞/6-5.png")