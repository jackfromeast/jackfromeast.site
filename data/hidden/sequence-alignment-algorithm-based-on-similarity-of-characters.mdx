---
title: '基于字符相似度的序列对齐算法'
date: '2022-02-07'
tags: ['自然语言处理', '算法']
draft: true
summary: '简介: 本篇文章将介绍基于相似度的Needleman-Wunsch序列对齐算法。该算法可用于在文字纠错、变体字识别中原文本与标注文本的对齐。'
---

简介:
本篇文章将介绍基于相似度的Needleman-Wunsch序列对齐算法。该算法可用于在文字纠错、变体字识别中原文本与标注文本的对齐。
...
![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/1.png)

<br>

同样在中文纠错、变体字识别等任务场景中也会出现需要使用序列对齐算法来快速完成序列标注，为接下来的模型训练做好准备工作。

![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/2.png)
...
对于两序列$seq1= a_1a_2a_3...a_m$和$seq2=b_1b_2b_3...b_n$，定义序列相似度矩阵$Similarity$，其中$Similarity(i,j)$对应着**两序列 ($a_1a_2a_3...a_j$ 和 $b_1b_2b_3...b_i$) 对齐后序列最大相似度。**最终根据该矩阵即可获得两序列相似度最大的对齐方式。

#### 2.1 矩阵初始化

首先对该矩阵进行初始化。由于我默认`Seq1`为原序列，`Seq2`为标注序列，`Seq1`的冗余度较高，长度均大于`Seq2`，故使`Seq2`向`Seq1`对齐，即仅在`Seq2`中加入`—`。所以矩阵初始化如下所示。

![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/3.png)
...
![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/4.png)

<br>

根据该传播公式即可得到Similarity矩阵中各个位置的值。示例如下所示。

![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/5.png)
...
![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/6.png)
...
![](/static/images/sequence-alignment-algorithm-based-on-similarity-of-characters/7.png)
